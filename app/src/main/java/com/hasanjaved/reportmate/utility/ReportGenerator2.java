...
//        addressRow.getCell(1).setText("AIR TEMP: 27°C");
    ...
    private static String saveDocumentToDirectory(Context context, XWPFDocument document, String outputDirectory, String fileName) {
        try {
            ...
            if (outputDirectory.startsWith("/")) {
                // Absolute path
                directory = new File(Environment.DIRECTORY_DOCUMENTS);
            } else {
                // Relative path - create under app's external files directory
                File baseDir = context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS);
                if (baseDir == null) {
                    Log.e(TAG, "External storage not available");
                    return null;
                }
                directory = new File(baseDir, outputDirectory);
            }

            Log.d(TAG, "Attempting to create directory: " + directory.getAbsolutePath());
            ...
            if (!directory.exists()) {
                ...
                if (!created) {
                    ...
                    if (parent != null && !parent.exists()) {
                        ...
                        Log.d(TAG, "Parent directory creation: " + parentCreated);

                        if (parentCreated) {
                            created = directory.mkdir();
                            Log.d(TAG, "Retry directory creation: " + created);
                        }
                    }

                    if (!created) {
                        // Final fallback - use app-specific directory
                        Log.w(TAG, "Using fallback directory");
                        directory = new File(context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS), "Reports");
                        if (!directory.exists()) {
                            directory.mkdirs();
                        }
                    }
            ...
            if (!directory.exists()) {
                Log.e(TAG, "Directory does not exist after creation attempt: " + directory.getAbsolutePath());
                return null;
            }

            if (!directory.canWrite()) {
                Log.e(TAG, "Directory is not writable: " + directory.getAbsolutePath());
                return null;
            }

            Log.d(TAG, "Directory confirmed: " + directory.getAbsolutePath());

            // Ensure filename has no invalid characters
            String cleanFileName = fileName.replaceAll("[^a-zA-Z0-9._-]", "_");
            if (!cleanFileName.endsWith(".docx")) {
                cleanFileName += ".docx";
            }

            // Create output file
            File outputFile = new File(directory, cleanFileName);

            Log.d(TAG, "Creating file: " + outputFile.getAbsolutePath());
            ...
            // Emergency fallback - save to app's cache directory
            try {
                File cacheDir = context.getCacheDir();
                File fallbackFile = new File(cacheDir, fileName + ".docx");

                FileOutputStream out = new FileOutputStream(fallbackFile);
                document.write(out);
                out.close();
                document.close();

                Log.w(TAG, "Document saved to cache directory as fallback: " + fallbackFile.getAbsolutePath());
                return fallbackFile.getAbsolutePath();

            } catch (Exception fallbackException) {
                Log.e(TAG, "Fallback save also failed", fallbackException);
                return null;
            }
        }
    }
    ...
    private static String saveDocument(Context context, XWPFDocument document, String fileName) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                File reportsDir = new File(context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS), "ReportMate");
...
// Usage Examples:
/*
// 1. Save to custom directory (relative path)
String reportPath = ReportGenerator.generateElectricalInspectionReport(
    this,
    "ReportMate/NEL-SWGRTS",  // Relative path - will be created under app's Documents directory
    "ElectricalInspection_" + System.currentTimeMillis(),
    "/storage/emulated/0/Pictures/1748710167004.jpg"
);

// 2. Save to absolute directory
String absoluteDir = context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS) + "/MyReports";
String reportPath2 = ReportGenerator.generateElectricalInspectionReport(
    this,
    absoluteDir,
...
// 3. Validate directory before generating report
String targetDir = "ReportMate/NEL-SWGRTS";
if (ReportGenerator.isDirectoryValid(this, targetDir)) {
    String reportPath = ReportGenerator.generateElectricalInspectionReport(
...
// 4. Get absolute path for a relative directory
String relativePath = "ReportMate/NEL-SWGRTS";
String absolutePath = ReportGenerator.getAbsoluteDirectoryPath(this, relativePath);
Log.d("ReportGenerator", "Absolute path: " + absolutePath);

// 5. Save to ReportMate directory (backward compatibility)
...
try {
    String reportPath = ReportGenerator.generateElectricalInspectionReport(
        this,
        "ReportMate/NEL-SWGRTS",
        "NEL_Report_" + System.currentTimeMillis(),
        "/storage/emulated/0/Pictures/1748710167004.jpg"
    );

    if (reportPath != null) {
        Log.d("Success", "Report saved to: " + reportPath);
        // Show success message or open file
    } else {
        Log.e("Error", "Failed to generate report");
        // Show error message
    }
} catch (Exception e) {
    Log.e("Error", "Exception generating report", e);
}
*/
