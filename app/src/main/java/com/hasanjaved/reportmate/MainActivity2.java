...
//        ActivityCompat.requestPermissions(this, REQUIRED_PERMISSIONS, PERMISSION_REQUEST_CODE);
    ...
    private void generateDocxReport() {
        try {
            ...
            // Save document to ReportMate directory in public Documents
            saveDocumentToReportMate(document);
    ...
    /**
     * Save document to ReportMate directory in public Documents using MediaStore
     */
    private void saveDocumentToReportMate(XWPFDocument document) {
        try {
            // Generate filename with timestamp
            String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(new Date());
            String fileName = "Panel_Board_Test_Report_" + timeStamp + ".docx";

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                // Android 10+ - Use MediaStore
                saveWithMediaStore(document, fileName);
            } else {
                // Legacy Android - Direct file access
                saveLegacy(document, fileName);
            }

        } catch (Exception e) {
            e.printStackTrace();
            Utility.showLog("Error saving document: " + e.getMessage());
     ...
     */
    private void saveWithMediaStore(XWPFDocument document, String fileName) {
        try {
            ...
            if (uri != null) {
                ...
                if (outputStream != null) {
                    ...
                    Toast.makeText(this, "Document saved to Documents/ReportMate/" + fileName, Toast.LENGTH_LONG).show();
            ...
            e.printStackTrace();
            Utility.showLog("MediaStore save failed, trying legacy method: " + e.getMessage());
     ...
     */
    private void saveLegacy(XWPFDocument document, String fileName) {
        try {
            ...
            if (!reportMateDir.exists()) {
                ...
                if (!created) {
                    throw new IOException("Failed to create ReportMate directory");
                }
            }
            ...
            e.printStackTrace();
            Utility.showLog("Legacy save failed: " + e.getMessage());
            ...
            try {
                ...
                Toast.makeText(this, "Document saved to Downloads/ReportMate/" + fileName, Toast.LENGTH_LONG).show();
                Utility.showLog("Document saved to Downloads fallback: " + docFile.getAbsolutePath());

            } catch (Exception fallbackException) {
                fallbackException.printStackTrace();
                Toast.makeText(this, "Failed to save document: " + fallbackException.getMessage(), Toast.LENGTH_LONG).show();
